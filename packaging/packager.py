            ...
            "\n".join(
                [
                    ...
                    *( [f'Name: "{{commondesktop}}\\{APP_NAME}"; Filename: "{{app}}\\{APP_NAME}.exe"; IconFilename: "{{app}}\\drunken_bot_logo_small.ico"'] if icon_path else [] ),
                    ...
                    "    WinHttp.Open('HEAD', 'https://download.pytorch.org/whl/cu124/', False);",
                    "    WinHttp.Send();",
                    "    Result := True;",
                    "  except",
                    "    Result := False;",
                    ...
                    "begin",
                    "  Result := True;",
                    "  if not InternetAvailable() then begin",
                    "    MsgBox('DrunkenBot-IDE needs an active internet connection to download the PyTorch runtime during installation. Please connect to the internet and run the installer again.', mbCriticalError, MB_OK);",
                    "    Result := False;",
                    "  end;",
                    "end;",
                ]
